...
const activity = [
    ...
    {id: 1, label: "Deposit - GHS 200", status:"done" ,date: "Jul 7"},
    {id: 1, label: "Credit request - Samsung Televison 43 -inch", status:"pending", date: "Jul 24"},
];

const statusStyles = {
    done: "bg -brand-50 text-brand-700 border-brand-100",
    pending: "bg-warn-100 text-warn-500 border-warn-500/300",
};

export default function Dashboard() {
    // replace with real user/plan data once backend is live !
    ...
    return (
        <div className="min-h-screen bg-page">
            <header className="flex item-center justify-between p-6 max-w-2xl mx-auto">
                <h1 className="text-xl">Hi, {user.firstName}</h1>
                <button
                arial-label="Notifications"
                className="w-9 h-9 rounded-full border border-line flex item-center justify-center"
                >
                  🔔
                </button>
            </header>

            <main className="max-w-2xl mx-auto px-6 pb-10 flex flex-col gap-6">
                {/* Balance card */}
                <Card varaint= "solid">
                ...
                <div className="flex gap-3">
                    ...
                    <Button varaint="outline" className="flex-1">
                        New plan
                    </Button>
                </div>
                {/* Recent activity */}
                <div>
                    ...
                    <div className="flex flex-col gap-2">
                        {activity.map((item) =>(
                            <Card key={item.id} className="flex item-center justify-between py-3">
                                <div>
